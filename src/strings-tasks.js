...
function removeFirstOccurrences(str, value) {
  ...
  const substr = str.slice(0, indx);
  const substr1 = str.slice(indx + value.length, str.length);
...
function sumOfCodes(str) {
  ...
  let sum = 0;
  for (let i = 0; i <= strin.length - 1; i += 1) {
    sum += strin[i].charCodeAt();
  }
  return sum;
}
...
function formatTime(minutes, seconds) {
  ...
  return minutes1.concat(':', seconds1);
}
...
function countVowels(str) {
  ...
  const arr = str.match(/[aeiouyAEIOUY]/g);
  if (arr === null) {
    return 0;
  }
  return arr.length;
...
function isPalindrome(str) {
  ...
  const str1 = str.toLowerCase().replace(regul, '');
...
function findLongestWord(sentence) {
  ...
  for (let i = 0; i < sentence1.length; i += 1) {
    if (sentence1[i].length > wordIndx) {
      wordIndx = sentence1[i].length;
      word = sentence1[i];
    }
  }
  return word;
...
function reverseWords(str) {
  ...
  for (let i = 0; i < str1.length; i += 1) {
    str2[i] = str1[i].split('').reverse().join('');
  }
  joinStr = str2.join(' ');
  return joinStr;
}
...
function invertCase(str) {
  ...
  let res = '';
  for (let i = 0; i < str1.length; i += 1) {
    const up = str1[i].toUpperCase();
    if (str1[i] === up) {
      strNew[i] = str1[i].toLowerCase();
    } else {
      strNew[i] = up;
    }
  }
...
function encodeToRot13(str) {
  ...
  for (let i = 0; i < str.length; i += 1) {
    const indx = alpha.indexOf(str[i]);
    res += indx > -1 ? alphaROT[indx] : str[i];
  }
  return res;
}
